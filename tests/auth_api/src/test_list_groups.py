...
def test_create_delete_group():
    ...
    ans = requests.get(f"http://{AUTH_API_HOST}/v1/group/{gid}/", headers={'Authorization': 'Bearer ' + token})
    ...
    assert ans.status_code == 200
    # Удаляем группу
    ans = requests.delete(f"http://{AUTH_API_HOST}/v1/group/{gid}/", headers={'Authorization': 'Bearer ' + token})
    assert ans.status_code == 200
